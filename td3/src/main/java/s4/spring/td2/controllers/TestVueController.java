...
public class TestVueController {
	...
	public String test(Model model) {
		...
		vue.addData("message", "Hello world !"); // on peut instancier une instance de modele/arraylist de modele
	...
	public String geneSpaOrgas(Model model) {
		...
		vue.addMethod("save", "var self=this;this.editedIndex=-1;this.dialog=false;" + Http.post("/rest/orgas/create", "self.editedItem",""));
		vue.addMethod("deleteItem", "var self=this;" + Http.post("/rest/orgas/delete", "item",JsArray.remove("self.orgas", "item")), "item");
		vue.addMethod("editItem", "this.editedIndex=1;this.editedItem.name=item.name;this.editedItem.domaine=item.domaine;this.editedItem.aliases=item.aliases;", "item");
		return "vueJS/index";
	}
}
